...
def inform_gain(data_vec):
    ...
    for subframe in data_vec:
        total_rows += float(len(subframe.index))       # now total_rows = total # data
    ...
    for n in range(0,len(unique_target_vals)):
        p_target_vals.append(float(target_val_freq[n]) / float(sum(target_val_freq)))
    ...
    for n, subframe in enumerate(data_vec):
        subframe_size = float(len(subframe.index))
        subframe_prob = float(subframe_size) / float(total_rows)
