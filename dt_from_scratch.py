...
df = pd.read_excel("/Users/ahmadauliawiguna/Documents/WORK/Riset/pemilu_as_2.xls", header=0)
dataset = df
...
def print_tree(node, depth=0):
	if isinstance(node, dict):
		...
		print('%s[%s = %.3f]' % ((depth*'-', (header[node['index']]), node['value'])))
		print_tree(node['left'], depth+3)
		print_tree(node['right'], depth+3)
	else:
